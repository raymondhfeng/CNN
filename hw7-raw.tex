...
\graphicspath{
	{C:/Users/Raymond/Downloads/Homeworks_Export (8)/HW7}
}
...
\begin{tcolorbox}[breakable]
	\[
	\includegraphics[width=0.6\textwidth]{init.png}
	\includegraphics[width=0.389\textwidth]{forward.png}
	\]
	\[
...
\begin{tcolorbox}[breakable]
	It appears that for some reason, the training loss is not decreasing over the minibatches, because, as the next part mentions, the gradients are not being useful. 
	\[
	\includegraphics[width=0.9\textwidth]{myloss.png}
	\]
	\[
...
\begin{tcolorbox}[breakable]
	We start with $R(p)$. 
	$$R(p)=\E_{\vec x}\left[\max\{0,1-f_{\vec p}(\vec x)\cdot (-1)^{\vec v_*^\top \vec x}\} \right]$$
	Then, we use the fact that because $x$ is over a discrete probability distribution, and so we can move the gradient to the inside of the expectation. 
	$$\nabla_pR(p)=\E_{\vec x}\left[ \nabla_p \max\{0,1-f_{\vec p}(\vec x)\cdot (-1)^{\vec v_*^\top \vec x}\} \right]$$
	Then, we can further simplify the expectation by simplifying the max, which we know will only be nonzero if $1-f_{\vec p}(\vec x)\cdot (-1)^{\vec v_*^\top \vec x}$ is positive. 
	$$\nabla_pR(p)=\E_{\vec x}\left[ -\nabla_p f_{\vec p}(\vec x)\cdot y | f_{\vec p}(\vec x)\cdot y \leq 1 \right]$$
	Then, using the defined indicator function, as well as the properties of conditional expectation, we can further simplify:
	$$\nabla_pR(p)=\E_{\vec x}\left[ -\nabla_p f_{\vec p}(\vec x)\cdot y \cdot \1_1(f_{\vec p}(\vec x)) \cdot y\right]$$
	Then, we can use the provided identity:
	$$\nabla_pR(p)=-\E_{\vec x}\left[\nabla_p f_{\vec p}(\vec x)\cdot \1_1(f_{\vec p}(\vec x)) \cdot y|y=1\right]+$$
	$$\E_{\vec x}\left[\nabla_p f_{\vec p}(\vec x)\cdot \1_1(-f_{\vec p}(\vec x)) \cdot y|y=-1\right]$$
	Then, using symmetry, we know that both events will happen half the time, which gives us the final expression:
	$$-\E_{\vec x}\left[ \frac{\1_1(f_{\vec p}(\vec x))-\1_1(-f_{\vec p}(\vec x))}{2} \nabla_{\vec p}f_{\vec p}(\vec x) \right]$$
	Furthermore, we argue that because functions of the form $(-1)^{\vec v_*^\top \vec x}$ are sub orthonormal, then we can define $g(x)$ to return the result of $(-1)^{\vec v_p^\top \vec x}$ in each of the elements, so long as $v_p \neq v_*$. This guarantees the term to be zero, as desired. 
\end{tcolorbox}
...
\begin{tcolorbox}[breakable]
	TODO: Explain differences between visualizations. 
	\[
	\includegraphics[scale=0.5]{pca.png}
	\includegraphics[scale=0.5]{cca.png}
	\]
\end{tcolorbox}
