...
class DataGen():
    ...
    def get_batch(self, N):
        ...
        Y = -2*Y+1 # rescaling
        return torch.from_numpy(X.astype(np.float32)).float(),torch.from_numpy(Y.astype(np.float32)).long()
...
class Net(nn.Module):
    def __init__(self, d):
        ...
        # self.b = np.random.rand(k,1)
        # self.alpha = np.random.rand(k,1)
        # self.beta = np.random.rand(1)
        self.W = nn.Linear(d,k)
        self.last = nn.Linear(k,1)
...
w = np.zeros((10*d,d))
b = np.zeros(10*d)
alpha = np.zeros((10*d,1))
beta = np.array([-1.0])
for i in range(int(3*d/2)):
    w[i,:] = data.v_star
...
print(sum(net(inputs).reshape(-1) - labels.float()))
...
def myLoss(outputs, labels):
    ...
    return (1/len(outputs))*torch.sum(torch.clamp(1-outputs*labels.float(),0))
